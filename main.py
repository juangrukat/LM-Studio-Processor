...
def process_files(server_url, prompt, folder, progress_bar, recursive=False, window=None) -> bool:
    ...
    for i, file_path in enumerate(files):
        ...
        try:
            ...
            try:
                response = query_llm(server_url, full_prompt, timeout=30)
        ...
        if event == sg.WIN_CLOSED:
            ...
            try:
                completed = process_files(server_url, prompt, values["files_folder"], 
                                       progress_bar, values["recursive_search"], window)
                if completed:
